...
// envFlagTrue returns true when key is set to 1/true/yes/on (case-insensitive).
...
func (r *Runner) Run() (*Report, error) {
	...
	var ari *ariProbe
	var ariItem Item
	if envFlagTrue("AAVA_AGENT_CHECK_SKIP_ARI") {
		ariItem = Item{
			Name:    "ARI",
			Status:  StatusSkip,
			Message: "probe skipped (AAVA_AGENT_CHECK_SKIP_ARI)",
			Details: "ARI reachability was not tested. Unset AAVA_AGENT_CHECK_SKIP_ARI when Asterisk ARI should be validated.",
		}
		ari = nil
	} else {
...
`
	...
	if !res.OK {
		...
		remediation := "Fix YAML syntax in config/ai-agent.yaml"
		if strings.Contains(details, "Permission denied") {
			remediation = "On the host run: chmod 644 config/ai-agent.local.yaml (so the container user can read it)"
		}
		return &res.Summary, Item{Name: "Config", Status: StatusFail, Message: msg, Details: details, Remediation: remediation}
	}
	details := []string{

...
This command:
  ...
  - Verifies success by running agent check (optional; use --skip-check or AAVA_AGENT_CHECK_SKIP_ARI=1 if ARI is down)
...
func printUpdateFailureRecovery(ctx *updateContext, err error) {
	...
	if err != nil {
		...
		if strings.Contains(lower, "check") {
			fmt.Println("Hint: If Asterisk/ARI is not running, re-run with --skip-check, or:")
			fmt.Println("  export AAVA_AGENT_CHECK_SKIP_ARI=1   # skips only the ARI probe in agent check")
		}
	}

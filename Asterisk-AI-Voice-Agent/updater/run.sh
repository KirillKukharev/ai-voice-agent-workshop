...
# When true, do not run `agent check` after update (e.g. Asterisk/ARI not running).
SKIP_CHECK="${AAVA_UPDATE_SKIP_CHECK:-false}" # true|false
...
run_update() {
  ...
  local -a upd_args=(update -v --self-update=false --remote="${REMOTE}" --ref="${REF}" --checkout="${CHECKOUT}" --backup-id="${JOB_ID}" --include-ui="${INCLUDE_UI}")
  if [ "${SKIP_CHECK}" = "true" ]; then
    upd_args+=(--skip-check)
    echo "==> Skipping post-update agent check (AAVA_UPDATE_SKIP_CHECK=true)" >&2
  fi

  set +e
  if [ -x "${BUILTIN_AGENT}" ]; then
    "${BUILTIN_AGENT}" "${upd_args[@]}" 2>&1 | tee "${JOB_LOG_PATH}"
  else
    "${AGENT_BIN}" "${upd_args[@]}" 2>&1 | tee "${JOB_LOG_PATH}"
  fi
  code="${PIPESTATUS[0]}"
  set -e

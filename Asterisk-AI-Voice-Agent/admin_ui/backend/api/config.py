...
def _get_env_key_from_dotenv(key_name: str) -> str:
    """Read a single KEY=value from project .env (same semantics as provider connection test)."""
    if not os.path.exists(settings.ENV_PATH):
        return ""
    try:
        with open(settings.ENV_PATH, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
...
def _resolve_env_in_str(value: Optional[str]) -> str:
    """Expand ${VAR:-default} patterns using .env then os.environ."""
    ...
    def repl(m: re.Match) -> str:
        ...
        v = _get_env_key_from_dotenv(var_name)
        if v:
            return v
    ...
    return re.sub(pattern, repl, value).strip()
...
def _write_local_config(content: str) -> None:
    ...
    # Backup existing local file (skip if not readable — e.g. wrong ownership on host)
    if os.path.exists(settings.LOCAL_CONFIG_PATH):
        try:
            ...
            # File exists but container user cannot read it; continue to try write (may still fail)
            pass

    # Preserve permissions from existing local or base file (ignore if not readable)
    original_mode = None
    try:
...
async def update_yaml_config(update: ConfigUpdate):
    try:
        ...
        logger.exception("POST /api/config/yaml permission denied: %s", e)
        raise HTTPException(
            status_code=503,
            detail=(
                "Permission denied writing config. On the host run: "
                "chown -R 1000:1000 config/  (or fix ownership of config/ and ai-agent.local.yaml so the container user can write)."
            ),
        ) from e
    except Exception as e:
        logger.exception("POST /api/config/yaml failed: %s", e)
...
class OpenRouterModelsRequest(BaseModel):
    """Optional api_key from the form; may be ${OPENROUTER_API_KEY} or a literal key."""

    api_key: Optional[str] = None


@router.post("/providers/openrouter-models")
async def list_openrouter_models(request: OpenRouterModelsRequest):
    """Fetch model catalog from OpenRouter (OpenAI-compatible GET /v1/models)."""
    raw = (request.api_key or "").strip()
    if raw:
        resolved = _resolve_env_in_str(raw)
    else:
        resolved = (
            os.getenv("OPENROUTER_API_KEY", "").strip()
            or _get_env_key_from_dotenv("OPENROUTER_API_KEY").strip()
        )
    if not resolved:
        raise HTTPException(
            status_code=400,
            detail="OpenRouter API key missing. Set OPENROUTER_API_KEY in .env or enter the key in the form.",
        )
    import httpx
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            r = await client.get(url, headers={"Authorization": f"Bearer {resolved}"})
    ...
    if r.status_code != 200:
        raise HTTPException(
            status_code=502,
            detail=f"OpenRouter /models returned HTTP {r.status_code}: {(r.text or '')[:240]}",
        )
    try:
        payload = r.json()
    except Exception as exc:
        raise HTTPException(status_code=502, detail="Invalid JSON from OpenRouter /models") from exc
    ...
    for item in rows:
        ...
        models.append({"id": str(mid), "name": str(label)})

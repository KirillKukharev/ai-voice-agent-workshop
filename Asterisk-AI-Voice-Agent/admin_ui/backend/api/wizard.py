...
def _is_git_lfs_pointer_file(path: str, max_check: int = 256) -> bool:
    """True if `path` is a Git LFS pointer text file (common HF mistake without LFS)."""
    try:
        with open(path, "rb") as f:
            head = f.read(max_check)
    except OSError:
        return False
    return head.startswith(b"version https://git-lfs.github.com/spec/v1")


def _hf_mirror_url(url: str) -> str:
    """Map huggingface.co resolve URL to hf-mirror.com (same path)."""
    if "huggingface.co" not in url:
        return url
    return url.replace("https://huggingface.co/", "https://hf-mirror.com/", 1).replace(
        "http://huggingface.co/", "https://hf-mirror.com/", 1
    )
...
class SingleModelDownload(BaseModel):
    ...
    download_url: str
    download_url_fallbacks: Optional[List[str]] = None  # Tried in order after primary fails / LFS stub
...
async def download_single_model(request: SingleModelDownload):
    ...
    def download_worker():
        try:
            ...
            candidate_urls: List[str] = [request.download_url]
            extras = request.download_url_fallbacks or []
            for u in extras:
                if u and u not in candidate_urls:
                    candidate_urls.append(u)
            # Auto-append HF mirror when primary is huggingface.co (no duplicate)
            mirror_auto = _hf_mirror_url(request.download_url)
            if mirror_auto != request.download_url and mirror_auto not in candidate_urls:
                candidate_urls.append(mirror_auto)

            temp_file: Optional[str] = None
            last_fetch_err: Optional[BaseException] = None
            for try_url in candidate_urls:
                temp_file = os.path.join(
                    target_dir, f".{temp_label}.{uuid.uuid4().hex}.download{ext}.part"
                )
                ...
                def progress_hook(block_num, block_size, total_size):
                    ...
                    if total_size > 0:
                        ...
                        if current_time - last_update_time >= 2:
                            ...
                            _job_output(
                                job.id,
                                f"   {mb_done:.1f}/{mb_total:.1f} MB ({percent}%) - {speed_mbps:.2f} MB/s - ETA: {eta_str}",
                            )

                try:
                    _job_output(job.id, f"   Trying: {try_url}")
                    urllib.request.urlretrieve(try_url, temp_file, progress_hook)
                    if (not is_archive) and ext == ".onnx" and _is_git_lfs_pointer_file(temp_file):
                        _job_output(
                            job.id,
                            "⚠️ Received Git LFS pointer instead of binary; retrying next mirror/URL...",
                        )
                        try:
                            os.remove(temp_file)
                        except OSError:
                    ...
                    last_fetch_err = fetch_exc
                    _job_output(job.id, f"⚠️ Download failed from this URL: {fetch_exc}")
                    try:
            ...
            if not temp_file:
                msg = (
                    str(last_fetch_err)
                    if last_fetch_err
                    else "All download URLs failed or returned Git LFS stubs (try hf-mirror, VPN, or: huggingface-cli download t-tech/T-one)"
                )

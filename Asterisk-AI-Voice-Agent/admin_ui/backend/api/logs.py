...
async def get_container_logs(
    ...
    try:
        ...
        # Prefer running container so we get live logs; otherwise first match (e.g. stopped)
        running = [c for c in containers if c.status == "running"]
        container = (running[0] if running else containers[0])
        
        wanted_levels = {v.strip().lower() for v in _split_csv(levels)} if levels else set()
        q_norm = (q or "").strip().lower() or None

        # Get logs (Docker may return empty for just-started or driver quirks; prefer running container already chosen above)
        try:
            logs_raw = container.logs(tail=tail)
        except Exception as log_err:
            raise HTTPException(
                status_code=500,
                detail=f"Failed to read container logs: {log_err}. Check Docker socket access and container status.",
            )
        logs = (logs_raw or b"").decode("utf-8", errors="replace")
        if not logs.strip() and container.status == "running":
            logs = f"[Container {container.name} is running but produced no log output yet. Try Refresh in a few seconds.]"

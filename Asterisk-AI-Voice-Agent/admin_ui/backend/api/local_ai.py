...
def _looks_like_tone_stt_dir(dirname: str) -> bool:
    """Match t-one / tone install folders (allow t_one, prefix t-one-*, etc.)."""
    n = (dirname or "").lower().replace("_", "-").strip()
    if n in {"t-one", "tone"}:
        return True
    return n.startswith("t-one")
    ...
    if os.path.exists(stt_dir):
        for item in os.listdir(stt_dir):
            ...
            if os.path.isdir(item_path):
                ...
                elif _looks_like_tone_stt_dir(item):
        ...
        # Expose kenlm.bin paths so catalog model_path "t-one/kenlm.bin" matches (basename kenlm.bin)
        tone_dirs_only = [
            m for m in stt_models["tone"]
            if m.path and not str(m.path).rstrip("/").endswith("kenlm.bin")
        ]
        for m in tone_dirs_only:
            p = str(m.path)
            if not p.startswith("/app/models/"):
                continue
            rel = p[len("/app/models/") :].lstrip("/")
            host = os.path.join(PROJECT_ROOT, rel)
            klm = os.path.join(host, "kenlm.bin")
            if os.path.isfile(klm):
                parent = os.path.basename(host.rstrip("/"))
                docker_klm = f"/app/models/stt/{parent}/kenlm.bin"
                if not any(str(x.path) == docker_klm for x in stt_models["tone"]):
                    stt_models["tone"].append(ModelInfo(
                        id=f"tone_{parent}_kenlm",
                        name=f"T-one KenLM ({parent})",
                        path=docker_klm,
                        type="stt",
                        backend="tone",
                        size_mb=get_file_size_mb(klm),
                    ))
...
async def switch_model(request: SwitchModelRequest):
    ...
    previous_env = _read_env_values(env_file, [
        ...
        "SILERO_TTS_ULAW_OUTPUT", "SILERO_TTS_DELIVERY_SAMPLE_RATE",

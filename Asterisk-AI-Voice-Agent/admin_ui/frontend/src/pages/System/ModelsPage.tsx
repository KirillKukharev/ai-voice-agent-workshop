...
const ModelsPage = () => {
    ...
    const handleDownload = async (model: ModelInfo, type: 'stt' | 'tts' | 'llm') => {
        ...
        try {
            const startRes = await axios.post('/api/wizard/local/download-model', {
                ...
                download_url_fallbacks: model.download_url_fallbacks,
    ...
    const normFsPath = (p: string) => (p || '').replace(/\\/g, '/');

    const isModelInstalled = (modelPath: string, catalogModel?: ModelInfo) => {
        const key = normalizeModelKey(modelPath);
        if (key && installedModels.some(m => normalizeModelKey(m.path) === key)) {
            return true;
        }

        // T-one: catalog uses "t-one" but installed API may list .../t-one/model.onnx or .../t-one only
        if (catalogModel?.backend === 'tone') {
            if (catalogModel.id === 'tone_ru' || key === 't-one') {
                return installedModels.some(m => {
                    const p = normFsPath(m.path);
                    return (
                        p.endsWith('/t-one') ||
                        p.endsWith('/t-one/model.onnx') ||
                        p.includes('/t-one/') ||
                        /\/t_one(\/|$)/.test(p)
                    );
                });
            }
            if (catalogModel.id === 'tone_ru_kenlm' || (modelPath || '').includes('kenlm')) {
                return installedModels.some(m => {
                    const p = normFsPath(m.path);
                    return p.includes('/t-one/kenlm.bin') || p.includes('/t_one/kenlm.bin');
                });
            }
        }
                                        ...
                                        {filterByRegion(catalog.stt).map(model => (
                                            <ConfigCard key={model.id}>
                                                <div className="flex justify-between items-center">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                {!model.auto_download && isModelInstalled(model.model_path || '', model) && (
                                                    ...
                                                    {!isModelInstalled(model.model_path || '', model) && model.download_url && (
                                                        <button
                                        ...
                                        {filterByRegion(catalog.tts).map(model => (
                                            <ConfigCard key={model.id}>
                                                <div className="flex justify-between items-center">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                {isModelInstalled(model.model_path || '', model) && (
                                                                    <span className="px-2 py-0.5 text-xs bg-green-100 dark:bg-green-900/30 text-green-600 rounded-full">
                                                                        Installed
                                                                    </span>
                                                                )}
                                                    ...
                                                    {!isModelInstalled(model.model_path || '', model) && model.auto_download && !model.download_url && (
                                                        <div className="flex flex-col items-end gap-1">
                                                            <button
                                                                onClick={() => openRebuildDialog(

...
const UpdatesPage = () => {
  ...
  const [skipAgentCheck, setSkipAgentCheck] = useState(false);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="flex flex-col justify-end gap-2">
              ...
              <label className="flex items-start gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={skipAgentCheck}
                  onChange={(e) => setSkipAgentCheck(e.target.checked)}
                  className="rounded border-border mt-0.5"
                />
                <span>
                  Skip post-update agent check (use when Asterisk/ARI is not running; update still applies)
                </span>
              </label>
            </div>
          </div>

...
const OpenAIProviderForm: React.FC<OpenAIProviderFormProps> = ({ config, onChange }) => {
    ...
    const chatBase = (config?.chat_base_url || '').toLowerCase();
    const isOpenRouter = name.includes('openrouter') || chatBase.includes('openrouter.ai');
    ...
    const filteredOrModels = useMemo(() => {
        ...
        if (!q) return orModels.slice(0, 200);
        return orModels.filter((m) => m.id.toLowerCase().includes(q)).slice(0, 250);
    }, [orModels, orFilter]);
    ...
    const fetchOpenRouterModels = async () => {
        ...
        setOrError(null);
        try {
            const res = await axios.post('/api/config/providers/openrouter-models', {
            ...
            setOrError(typeof msg === 'string' ? msg : 'Failed to load models');
            setOrModels([]);
        } finally {
            setOrLoading(false);
        }
    ...
    const defaultKeyPlaceholder = isOpenRouter ? '${OPENROUTER_API_KEY}' : '${OPENAI_API_KEY}';
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            value={config.api_key || defaultKeyPlaceholder}
                    ...
                    {isOpenRouter && (
                        <div className="md:col-span-2 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
                            OpenRouter uses the Chat Completions API only here. Organization, project, and Realtime API settings are hidden.
                            Optional headers (<code>HTTP-Referer</code>, <code>X-OpenRouter-Title</code>) stay in YAML under{' '}
                            <code>extra_http_headers</code> if configured.
                        </div>
                    )}
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {isOpenRouter ? (
                            <div className="space-y-2 md:col-span-2">
                                <label className="text-sm font-medium">Chat model (OpenRouter id)</label>
                                <input
                                    type="text"
                                ...
                                <p className="text-xs text-muted-foreground">
                                    ...
                                    . Example: <code>nvidia/nemotron-3-nano-30b-a3b:free</code>
                                </p>
                                <div className="flex flex-wrap items-center gap-2 pt-1">
                                    <button
                                        ...
                                        disabled={orLoading}
                                    >
                                        {orLoading ? 'Loading…' : 'Refresh model list'}
                                    </button>
                                    {orModels.length > 0 && (
                                        <span className="text-xs text-muted-foreground">{orModels.length} models loaded</span>
                                    )}
                                </div>
                                {orError && <p className="text-xs text-destructive">{orError}</p>}
                                {orModels.length > 0 && (
                                    <div className="space-y-2 pt-2">
                                        <label className="text-sm font-medium">Pick from catalog (filter)</label>
                                        <input
                                            type="text"
                                            className="w-full p-2 rounded border border-input bg-background"
                                            value={orFilter}
                                            onChange={(e) => setOrFilter(e.target.value)}
                                            placeholder="Filter e.g. nemotron, :free, openai/…"
                                        />
                                        <select
                                            className="w-full p-2 rounded border border-input bg-background max-h-40 font-mono text-sm"
                                            size={Math.min(12, Math.max(4, filteredOrModels.length))}
                                            value={
                                                filteredOrModels.some((m) => m.id === config.chat_model)
                                                    ? config.chat_model
                                                    : ''
                                            }
                                            onChange={(e) => {
                                                const v = e.target.value;
                                                if (v) handleChange('chat_model', v);
                                            }}
                                        >
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            Showing up to {filteredOrModels.length} matches. Narrow the filter if the list is truncated.
                                        </p>
                                    </div>
                                )}
                            </div>

...
**4. STT always empty / Bot doesn't hear me** (logs: `STT FINAL SUPPRESSED - Empty transcript`, `has_final=0`, `pre_guard_pcm_rms=0`)

**Cause A – Inbound audio is silence:** If ai_engine logs show `pre_guard_pcm_rms=0` and `post_guard_pcm_rms=0` on every frame, the engine is receiving no real speech from Asterisk. The Local AI Server may still log `rms=32124` (decoded μ-law silence → constant PCM). Vosk correctly returns no text.

**Fix:**
- Ensure RTP from the phone reaches Asterisk (see [Asterisk Docker RTP Troubleshooting](Asterisk-Docker-RTP-Troubleshooting.md): `AST_EXTERNAL_IP`, port publishing, firewall).
- In Admin UI → Config → **AudioSocket** (or Transport), confirm **Format** matches what Asterisk sends (e.g. `ulaw` / μ-law 8kHz). Wrong format can yield zero or garbage PCM.
- Test with a known-working softphone and the same network path.

**Cause B – STT model language mismatch:** Vosk models are language-specific. If you speak Russian but the model is English (`vosk-model-en-us-0.22`), you will get empty or wrong transcripts.

**Fix:** Set the correct model in `.env` and ensure the container uses it:
```bash
# Russian speech
LOCAL_STT_MODEL_PATH=/app/models/stt/vosk-model-small-ru-0.22
```
Ensure the model is present under `./models/stt/` (or the path you mount into the container). Restart Local AI Server and check startup logs for:
```
✅ STT backend: Vosk loaded from .../vosk-model-small-ru-0.22 (16kHz native)
```

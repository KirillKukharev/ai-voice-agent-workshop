...
- `vosk-model-small-ru-0.22` (Russian; use for Russian speech)
...
- **Download issues:** Artifacts live on [t-tech/T-one](https://huggingface.co/t-tech/T-one). If `huggingface.co` is slow or blocked, the Admin UI model downloader retries **https://hf-mirror.com/** (same paths). Manual option: `huggingface-cli download t-tech T-one --local-dir ./models/stt/t-one` (install `huggingface_hub`; use `HF_TOKEN` if the hub requires sign-in). For Sherpa-ONNX packaging of a related Russian streaming model (different integration), see [csukuangfj/sherpa-onnx-streaming-t-one-russian-2025-09-08](https://huggingface.co/csukuangfj/sherpa-onnx-streaming-t-one-russian-2025-09-08) with `LOCAL_STT_BACKEND=sherpa`.

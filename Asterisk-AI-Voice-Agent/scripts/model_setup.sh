...
# Optional mirror URLs (Hugging Face CDN, often faster than alphacephei)
get_vosk_fallback_url() {
  case "$LANGUAGE" in
    en-US) echo "https://huggingface.co/rhasspy/vosk-models/resolve/main/en/vosk-model-small-en-us-0.15.zip" ;;
    *)     echo "" ;;
  esac
}
...
VOSK_FALLBACK_URL=$(get_vosk_fallback_url)
...
# Try multiple URLs in order until one succeeds (e.g. primary + mirror)
download_any() { # dest_path label url1 [url2 ...]
  local dest="$1" label="$2"
  shift 2
  mkdir -p "$(dirname "$dest")"
  for url in "$@"; do
    [ -z "$url" ] && continue
    echo "Downloading $label → $dest (from ${url%%/*}...)"
    if curl -L --retry 2 --connect-timeout 30 --max-time 600 -f -o "$dest" "$url" 2>/dev/null; then
      echo "  ✓ Success"
      return 0
    fi
    echo "  ✗ Failed or slow, trying next mirror..."
  done
  echo "ERROR: All download URLs failed for $label" >&2
  return 1
}
...
setup_light_cpu() {
  # STT (Vosk) - language-aware model (tries mirror if primary is slow/unreachable)
  local stt_zip="$MODELS_DIR/stt/${VOSK_MODEL}.zip"
  if [ ! -f "$MODELS_DIR/stt/${VOSK_MODEL}/README" ]; then
    if [ -n "$VOSK_FALLBACK_URL" ]; then
      download_any "$stt_zip" "${VOSK_MODEL}" "$VOSK_FALLBACK_URL" "$VOSK_URL"
    else

...
# Short utterances that strongly suggest a goodbye (any language); substring match after normalize.
_MULTILINGUAL_FAREWELL_FRAGMENTS: tuple[str, ...] = (
    "до свидания",
    "до встречи",
    "всего доброго",
    "всего хорошего",
...
# Terminal goodbyes suitable for assistant-only hangup (exclude "до встречи", which appears mid-booking).
_ASSISTANT_ONLY_TERMINAL_FRAGMENTS: tuple[str, ...] = (
    "до свидания",
    "прощайте",
    "спасибо за звонок",
    "thank you for calling",
...
def assistant_short_terminal_farewell(response_text: str, max_words: int = 10) -> bool:
    """
    True when the assistant reply is short and clearly ends the call (RU/EN).

    Used when the model says goodbye without calling hangup_call and the caller did not
    match end_call markers (e.g. stayed silent after the bot's closing line).
    """
    t = " ".join((response_text or "").strip().lower().split())
    if not t or len(t.split()) > max_words:
        return False
    return any(frag in t for frag in _ASSISTANT_ONLY_TERMINAL_FRAGMENTS)


def text_contains_short_multilingual_farewell(text: str, max_words: int = 14) -> bool:
    """
    True when text is short and contains a common farewell fragment (RU/EN).
    Used for hangup guardrails and pipeline hangup heuristics where STT is not English-only.
    """
    ...
    if len(t.split()) > max_words:
        return False
    return any(frag in t for frag in _MULTILINGUAL_FAREWELL_FRAGMENTS)
...
def text_is_short_polite_closing(text: str) -> bool:
    ...
    if not normalized:
        return False
    if text_contains_short_multilingual_farewell(text, max_words=8):
        return True

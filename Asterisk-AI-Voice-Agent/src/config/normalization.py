...
def _generate_default_pipeline(config_data: Dict[str, Any]) -> None:
    ...
    dp = _pipeline_component_default_provider(config_data, None, default_provider)
    default_components = _compose_provider_components(dp)
    
    pipelines = config_data.setdefault("pipelines", {})
    existing_entry = pipelines.get(pipeline_name)
    
    if existing_entry is None:
        pipelines[pipeline_name] = _compose_provider_components(dp)
...
# Provider tokens that are valid for _compose_provider_components (not pipeline names).
_PIPELINE_DEFAULT_PROVIDER_SLUGS = frozenset(
    {
        "local",
        "deepgram",
        "openai_realtime",
...
def _pipeline_component_default_provider(config_data: Dict[str, Any], provider_hint: Any, default_provider: Any) -> str:
    """
    When default_provider is a *pipeline name* (e.g. local_hybrid), do not use it to compose
    stt/llm/tts defaults — that would yield nonsense keys like local_hybrid_llm and force
    placeholder/local_llm fallbacks.
    """
    if provider_hint:
        return str(provider_hint).strip()
    dp = str(default_provider or "").strip()
    pipelines_cfg = config_data.get("pipelines") or {}
    if isinstance(pipelines_cfg, dict) and dp in pipelines_cfg:
        return "local"
    # Pipeline-first configs use default_provider=local_hybrid before/without pipelines block in edge cases
    if "_" in dp and dp not in _PIPELINE_DEFAULT_PROVIDER_SLUGS:
        return "local"
    return dp or "openai_realtime"
...
def normalize_pipelines(config_data: Dict[str, Any]) -> None:
    ...
    for pipeline_name, raw_entry in pipelines_cfg.items():
        if raw_entry is None:
            dp = _pipeline_component_default_provider(config_data, None, default_provider)
            normalized[pipeline_name] = _compose_provider_components(dp)
            continue
        ...
        if isinstance(raw_entry, dict):
            ...
            provider_for_defaults = _pipeline_component_default_provider(
                config_data, provider_hint, default_provider
            )

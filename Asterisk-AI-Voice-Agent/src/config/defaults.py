...
def apply_audiosocket_defaults(config_data: Dict[str, Any]) -> None:
    ...
    # Host (bind address) - env overrides YAML so Docker/NAT can bind 0.0.0.0
    env_host = os.getenv('AUDIOSOCKET_HOST', '').strip()
    if env_host:
        audiosocket_cfg['host'] = env_host
    else:
        audiosocket_cfg.setdefault('host', '127.0.0.1')
    ...
    # Port - env overrides YAML
    try:
        port_default = audiosocket_cfg.get('port', 8090)
        env_port = os.getenv('AUDIOSOCKET_PORT', '').strip()
        audiosocket_cfg['port'] = int(env_port) if env_port else port_default
    except ValueError:
        audiosocket_cfg['port'] = 8090
    
    # Format - env overrides YAML
    env_fmt = os.getenv('AUDIOSOCKET_FORMAT', '').strip()
    audiosocket_cfg['format'] = env_fmt or audiosocket_cfg.get('format', 'ulaw')

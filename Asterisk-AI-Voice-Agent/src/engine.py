...
from src.tools.telephony.hangup_policy import (
    ...
    text_contains_short_multilingual_farewell,
...
class Engine:
            ...
            try:
                ...
                try:
                    if self.config.audiosocket and hasattr(self.config.audiosocket, 'format'):
                        as_format = getattr(self.config.audiosocket, 'outbound_format', None) or self.config.audiosocket.format
                except Exception:
                    as_format = None
                self.streaming_playback_manager.set_transport(
                    audio_transport=self.config.audio_transport,
    ...
    def _timezone_for_prompt_substitution(self) -> ZoneInfo:
        """Clock for {today_*} / {current_datetime_*} placeholders (calendar-aligned when possible)."""
        names: List[str] = []
        try:
            tools_cfg = getattr(self.config, "tools", None) or {}
            if isinstance(tools_cfg, dict):
                gc = tools_cfg.get("google_calendar") or {}
                if isinstance(gc, dict):
                    z = (gc.get("timezone") or "").strip()
                    if z:
                        names.append(z)
        except Exception:
            pass
        for key in ("GOOGLE_CALENDAR_TZ", "TZ", "AAVA_SERVER_TIMEZONE"):
    ...
    def _format_conversation_history_for_prompt(
        ...
        *,
        ...
        """Turn session.conversation_history into a compact text block for system prompts."""
        hist = list(getattr(session, "conversation_history", None) or [])
        slice_hist = hist[-max(1, max_messages) :]
        lines: List[str] = []
        for msg in slice_hist:
            if not isinstance(msg, dict):
                continue
            role = str(msg.get("role") or "?").strip().lower() or "?"
            content = str(msg.get("content") or "").strip()
            if not content:
                continue
            content = re.sub(r"\s+", " ", content)
            if len(content) > line_max:
                content = content[: max(1, line_max - 1)] + "…"
            lines.append(f"{role}: {content}")
        text = "\n".join(lines)
        if len(text) > max_chars:
            text = "(truncated)\n" + text[-(max_chars - 20) :]
        return text.strip() or "(no prior turns in this session yet)"

    def _resolve_pipeline_system_prompt_template(self, session: CallSession, pipeline: Any) -> Optional[str]:
        """Raw system prompt template (before substitution): context → pipeline default → global llm.prompt."""
        cn = getattr(session, "context_name", None)
        if cn:
            cc = self.transport_orchestrator.get_context_config(cn)
            if cc and (getattr(cc, "prompt", None) or "").strip():
                return str(cc.prompt).strip()
        p_def = getattr(pipeline, "llm_options", None) or {}
        raw = (p_def.get("system_prompt") or "").strip()
        if raw:
            return str(p_def["system_prompt"])
        gp = getattr(self.config.llm, "prompt", None)
        if gp and str(gp).strip():
            return str(gp).strip()
        return None

    def _refresh_pipeline_system_prompt(
        self,
        session: CallSession,
        pipeline: Any,
        llm_options_in: Dict[str, Any],
    ) -> Dict[str, Any]:
        """Re-apply template substitution so {today_*}, {conversation_history}, etc. stay current each turn."""
        tpl = self._resolve_pipeline_system_prompt_template(session, pipeline)
        if not tpl:
            return llm_options_in
        out = dict(llm_options_in or {})
        sp = self._apply_prompt_template_substitution(tpl, session)
        try:
            if getattr(session, "is_outbound", False) and getattr(session, "outbound_custom_vars", None):
                sp = self._append_outbound_custom_vars_to_prompt(
                    sp,
                    getattr(session, "outbound_custom_vars", {}) or {},
                )
        except Exception:
            logger.debug("Outbound custom_vars re-append failed", call_id=session.call_id, exc_info=True)
    ...
    def _apply_prompt_template_substitution(
        ...
        - {today_iso}: Today's date as YYYY-MM-DD (timezone: tools.google_calendar.timezone, else GOOGLE_CALENDAR_TZ, TZ, AAVA_SERVER_TIMEZONE, UTC)
        - {today_date}: Same as {today_iso}
        - {today_readable_ru}: Today's date as DD.MM.YYYY
        - {current_datetime_iso}: Current local date-time in ISO 8601 for that timezone
        - {conversation_history}: Recent dialog lines from this call (user:/assistant:), for system prompt only
        - {dialog_history}: Alias for {conversation_history}
        ...
        try:
            tz = self._timezone_for_prompt_substitution()
            now = datetime.now(tz)
            substitutions["today_iso"] = now.date().isoformat()
            substitutions["today_date"] = substitutions["today_iso"]
            substitutions["today_readable_ru"] = now.strftime("%d.%m.%Y")
            substitutions["current_datetime_iso"] = now.isoformat(timespec="seconds")
            hist_txt = self._format_conversation_history_for_prompt(session)
            substitutions["conversation_history"] = hist_txt
            substitutions["dialog_history"] = hist_txt
        except Exception:
            logger.debug(
                "Prompt date/history substitution defaults failed",
                call_id=getattr(session, "call_id", None),
                exc_info=True,
            )
            substitutions.setdefault("today_iso", "")
            substitutions.setdefault("today_date", "")
            substitutions.setdefault("today_readable_ru", "")
            substitutions.setdefault("current_datetime_iso", "")
            substitutions.setdefault("conversation_history", "")
            substitutions.setdefault("dialog_history", "")
        ...
        try:
            ...
            if ok:
                logger.info(
                    ...
                    silence_ms=silence_ms,
                    talking_threshold=talking_thr,
                    is_pipeline=self._pipeline_session_active(call_id),
                )
            else:
    ...
    async def _handle_channel_talking_started(self, event: dict) -> None:
        ...
        try:
            ...
            # Only skip when not in an outbound-TTS window: gated capture, tts flag, active stream, or tokens.
            streams = getattr(self.streaming_playback_manager, "active_streams", {}) or {}
            stream_active = call_id in streams
            tts_tokens = int(getattr(session, "tts_active_count", 0) or 0) > 0
            if (
                bool(getattr(session, "audio_capture_enabled", True))
                and not bool(getattr(session, "tts_playing", False))
                and not stream_active
                and not tts_tokens
            ):
                return
            ...
            now = time.time()
            tts_elapsed_ms = self._tts_elapsed_ms_for_barge_in(session, now)
    ...
    async def _persist_call_metrics(self, session: CallSession) -> None:
        """Write turn_latencies_ms and barge_in_count to call_metrics table so they survive restarts/other workers."""
        try:
            from src.core.call_history import get_call_history_store
            store = get_call_history_store()
            if not store._enabled:
                return
            latencies = list(getattr(session, 'turn_latencies_ms', []) or [])
            count = int(getattr(session, 'barge_in_count', 0) or 0)
            await store.upsert_call_metrics(session.call_id, latencies, count)
        except Exception as e:
            logger.debug("Failed to persist call metrics", call_id=session.call_id, error=str(e))
    ...
    async def _persist_call_history(self, session: CallSession, call_id: str) -> None:
        ...
        try:
            ...
            # Prefer metrics from DB (call_metrics) so they are correct after restart or in multi-worker setups
            metrics = await store.get_call_metrics(call_id)
            if metrics is not None:
                turn_latencies, barge_in_count = metrics
            else:
            ...
            # Calculate latency stats (from DB or session)
            ...
            try:
                await store.delete_call_metrics(call_id)
            except Exception:
                logger.debug("Failed to delete call_metrics after persist", call_id=call_id)
            if saved:
    ...
    async def _audiosocket_handle_audio(self, conn_id: str, audio_bytes: bytes) -> None:
        ...
        try:
            ...
            if self._pipeline_session_active(caller_channel_id):
                # AAVA-28: Hold upstream while TTS is gated, flagged playing, or streaming playback still active
                # (post–AgentAudioDone tail under continuous_stream).
                if self._pipeline_hold_upstream_for_barge_in(session, caller_channel_id):
                    # Pipelines: allow barge-in detection during TTS gating, but do not forward audio until triggered.
                    cfg = getattr(self.config, "barge_in", None)
                    if not cfg or not getattr(cfg, "enabled", True):
                        return
                    # TALK_DETECT (ARI) and local energy both run: on AudioSocket, TALK_DETECT often never
                    # fires while the bot plays audio, which previously made barge-in impossible when this
                    # path returned early. Cooldown in _apply_barge_in_action avoids double-firing.
                    now = time.time()
                    tts_elapsed_ms = self._tts_elapsed_ms_for_barge_in(session, now)
                    ...
                    try:
                        ...
                        frame_ms = 20
                    self._pipeline_barge_accumulate_candidate(
                        session,
                        energy=energy,
                        frame_ms=frame_ms,
                        threshold=threshold,
                    ...
                    try:
                        ...
                        if now - last >= 1.0:
                            ...
                            logger.debug(
                                ...
                                energy=energy,
                                speech_ema=float(
                                    (session.vad_state.get("pipeline_barge_smooth") or {}).get("rms_ema", 0.0)
                                )
                                if isinstance(session.vad_state, dict)
                                else 0.0,
                                threshold=threshold,
                                ...
                                streaming_active=caller_channel_id
                                in getattr(self.streaming_playback_manager, "active_streams", {}),
                            )
                    except Exception:
                        pass
    ...
    @staticmethod
    def _tts_elapsed_ms_for_barge_in(session: CallSession, now: float) -> int:
        """Elapsed ms since TTS gating started for barge-in protection windows.

        If ``tts_started_ts`` was never set while gating is active, returning 0 would keep
        ``tts_elapsed_ms < initial_protect`` true forever and block all barge-in. Treat
        missing/invalid timestamps as "protection already elapsed".
        """
        try:
            ts = float(getattr(session, "tts_started_ts", 0.0) or 0.0)
        except (TypeError, ValueError):
            ts = 0.0
        if ts <= 0.0:
    ...
    def _pipeline_hold_upstream_for_barge_in(self, session: CallSession, call_id: str) -> bool:
        """True while pipeline caller audio should not go to STT: TTS gated, playing, or stream still draining.

        continuous_stream calls end_segment_gating on AgentAudioDone when the *provider* stops sending,
        but the jitter/pacer may still play seconds of audio — capture is already re-enabled, so barge-in
        must keep running until active_streams clears.
        """
        try:
            stream_id = getattr(session, "call_id", None) or call_id
            streams = getattr(self.streaming_playback_manager, "active_streams", {}) or {}
            if int(getattr(session, "tts_active_count", 0) or 0) > 0:
                return True
            if not session.audio_capture_enabled:
                return True
            if bool(getattr(session, "tts_playing", False)):
                return True
            if stream_id in streams or call_id in streams:
                return True
        except Exception:
            return bool(not getattr(session, "audio_capture_enabled", True))
        return False

    def _pipeline_session_active(self, caller_channel_id: str) -> bool:
        """True when this call uses the pipeline audio path (forced mode or an active pipeline queue)."""
        try:
            queues = getattr(self, "_pipeline_queues", {}) or {}
            return bool(self._pipeline_forced.get(caller_channel_id) or caller_channel_id in queues)
        except Exception:
            return bool(self._pipeline_forced.get(caller_channel_id))

    def _pipeline_barge_accumulate_candidate(
        self,
        session: CallSession,
        *,
        energy: int,
        ...
        """Update barge_in_candidate_ms during pipeline TTS (RMS is per-frame; speech fluctuates)."""
        try:
            alpha = float(getattr(cfg, "pipeline_barge_rms_ema_alpha", 0.22) or 0.0)
        except (TypeError, ValueError):
            alpha = 0.22
        if alpha <= 0.0:
            speech_metric = float(energy)
            try:
                if isinstance(session.vad_state, dict):
                    session.vad_state.pop("pipeline_barge_smooth", None)
            except Exception:
                pass
        else:
            try:
                st = session.vad_state.setdefault("pipeline_barge_smooth", {})
                ema = float(st.get("rms_ema", 0.0) or 0.0)
                ema = (1.0 - alpha) * ema + alpha * float(energy)
                st["rms_ema"] = ema
                speech_metric = max(float(energy), ema)
            except Exception:
                speech_metric = float(energy)

        if speech_metric >= float(threshold):
            if int(getattr(session, "barge_in_candidate_ms", 0)) == 0:
                try:
                    session.barge_start_ts = now
                except Exception:
                    session.barge_start_ts = 0.0
            session.barge_in_candidate_ms = int(getattr(session, "barge_in_candidate_ms", 0)) + int(frame_ms)
        else:
            try:
                decay_ratio = float(getattr(cfg, "pipeline_barge_silent_decay_ratio", 0.4) or 0.4)
            except (TypeError, ValueError):
                decay_ratio = 0.4
            decay_ratio = max(0.05, min(0.95, decay_ratio))
            decay = max(1, int(int(frame_ms) * decay_ratio))
            cur = int(getattr(session, "barge_in_candidate_ms", 0) or 0)
            session.barge_in_candidate_ms = max(0, cur - decay)
    ...
    async def _apply_barge_in_action(self, call_id: str, *, source: str, reason: str) -> None:
        ...
        try:
            ...
            try:
                ...
                await self._save_session(session)
                await self._persist_call_metrics(session)
            except Exception:
    ...
    async def _on_rtp_audio(self, caller_channel_id: str, ssrc: int, pcm_16k: bytes) -> None:
        ...
        try:
            ...
            pipeline_forced = self._pipeline_session_active(caller_channel_id)
            ...
            if pipeline_forced:
                # Same as AudioSocket: include streaming tail after segment gating ends.
                if self._pipeline_hold_upstream_for_barge_in(session, caller_channel_id):
                    # Pipelines: allow barge-in detection during TTS gating, but do not forward audio until triggered.
                    cfg = getattr(self.config, "barge_in", None)
                    if not cfg or not getattr(cfg, "enabled", True):
                        return
                    # Keep local energy barge-in even when TALK_DETECT is enabled (same as AudioSocket path).
                    now = time.time()
                    tts_elapsed_ms = self._tts_elapsed_ms_for_barge_in(session, now)
                    initial_protect = int(getattr(cfg, "initial_protection_ms", 200))
                    try:
        ...
        try:
            ...
            if etype == "AgentAudio":
                ...
                if continuous:
                    ...
                    # Pipeline + AudioSocket need fresh per-segment gating so capture stays off during
                    # each TTS segment; otherwise barge-in (energy VAD) never runs after the first segment.
                    # OpenAI/Deepgram keep _segment_tts_active to avoid re-gating churn with server-side AEC.
                    should_rearm_segment_gating = _prov in ("google_live", "local", "pipeline")
                ...
                if tool_calls and any((tc.get("name") or "").strip() == "hangup_call" for tc in tool_calls):
                    ...
                    if policy_mode != "relaxed":
                        ...
                        has_end_intent = (
                            ...
                            or text_contains_short_multilingual_farewell(user_text, max_words=12)
                        )
    ...
    async def _pipeline_runner(self, call_id: str) -> None:
        ...
        try:
            ...
            if not pipeline:
                ...
                return
            # AudioSocket and some SIP paths never set channel var AI_CONTEXT; Asterisk dialplan "context"
            # (e.g. [default]) is unrelated. Without AI_CONTEXT, pipeline tool allowlist was empty
            # (tools_count=0) while global llm.prompt still applied — models then hallucinate tool outcomes.
            try:
                raw_ctx = (getattr(session, "context_name", None) or "").strip()
                if not raw_ctx and self.transport_orchestrator.get_context_config("default"):
                    session.context_name = "default"
                    await self._save_session(session)
                    logger.info(
                        "Pipeline bound to contexts.default (AI_CONTEXT was unset)",
                        call_id=call_id,
                    )
            except Exception:
                logger.debug(
                    "Pipeline default context bind skipped",
                    call_id=call_id,
                    exc_info=True,
                )
            ...
            if greeting:
                ...
                for attempt in range(1, max_attempts + 1):
                    try:
                        greeting_turn_start = time.time()  # For call history: first turn = greeting
                        ...
                        try:
                            ...
                            tts_rate = 8000
                        # Ensure local TTS (mulaw 8k) is passed so streaming does mulaw→PCM, not wrong format
                        if (getattr(pipeline, "tts_key", "") or "").lower().startswith("local") and not tts_format:
                            tts_encoding, tts_rate = "mulaw", 8000

                        if use_streaming_playback:
                            q: asyncio.Queue = asyncio.Queue(maxsize=256)
                            tp = getattr(session, "transport_profile", None)
                            tgt_enc = getattr(tp, "wire_encoding", None) if tp else None
                            tgt_rate = getattr(tp, "wire_sample_rate", None) if tp else None
                            stream_id = await self.streaming_playback_manager.start_streaming_playback(
                                ...
                                target_sample_rate=tgt_rate,
                            )
                            if not stream_id:
                                raise RuntimeError("start_streaming_playback returned no stream_id")
                            any_audio = False
                            first_greeting_chunk_ts = None
                            async for chunk in pipeline.tts_adapter.synthesize(call_id, greeting, pipeline.tts_options):
                                if not chunk:
                                    continue
                                if call_id not in self.streaming_playback_manager.active_streams:
                                    break
                                if first_greeting_chunk_ts is None:
                                    first_greeting_chunk_ts = time.time()
                                    session.turn_latencies_ms.append((first_greeting_chunk_ts - greeting_turn_start) * 1000)
                                    await self._persist_call_metrics(session)
                                any_audio = True
                                await q.put(chunk)
                            if call_id in self.streaming_playback_manager.active_streams:
                                try:
                                    q.put_nowait(None)
                                except asyncio.QueueFull:
                                    asyncio.create_task(q.put(None))
            ...
            async def dialog_worker() -> None:
                ...
                async def run_turn(transcript_text: str) -> None:
                    nonlocal conversation_history, llm_options, session
                    # Reconcile with session if another code path updated history (avoids empty prior_messages).
                    try:
                        sess_hist = list(session.conversation_history or [])
                        if len(sess_hist) > len(conversation_history):
                            conversation_history = sess_hist
                    except Exception:
                        pass
                    # Refresh session + system prompt so {today_*}, {conversation_history}, etc. stay current each turn.
                    try:
                        fresh = await self.session_store.get_by_call_id(call_id)
                        if fresh is not None:
                            session = fresh
                            sh = list(fresh.conversation_history or [])
                            if len(sh) >= len(conversation_history):
                                conversation_history = sh
                        llm_options = self._refresh_pipeline_system_prompt(session, pipeline, llm_options)
                    except Exception:
                        logger.debug(
                            "Pipeline system_prompt refresh skipped",
                            call_id=call_id,
                            exc_info=True,
                        )
                    response_text = ""
                    ...
                    async def _pipeline_play_tts_audio(spoken: str, playback_type: str = "pipeline-tts") -> None:
                        """Play pipeline TTS via streaming when downstream is not ``file`` (required for AudioSocket)."""
                        text = (spoken or "").strip()
                        if not text:
                            return
                        _dm = getattr(pipeline.tts_adapter, "downstream_mode_override", "auto") or "auto"
                        if _dm == "stream":
                            use_stream = True
                        elif _dm == "file":
                            use_stream = False
                        else:
                            use_stream = self.config.downstream_mode != "file"
                        if not use_stream:
                            tts_bytes = bytearray()
                            ...
                            if tts_bytes:
                                pid = await self.playback_manager.play_audio(
                                    call_id, bytes(tts_bytes), playback_type
                                )
                                if pid:
                                    await self.playback_manager.wait_for_playback_end(
                                        call_id,
                                        pid,
                                        timeout_sec=(len(tts_bytes) / 8000.0 + 3.0),
                                    )
                            return

                        stream_q: asyncio.Queue = asyncio.Queue(maxsize=256)
                        old_pn = getattr(session, "provider_name", None)
                        try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                tts_rate = 8000
                            if (getattr(pipeline, "tts_key", "") or "").lower().startswith(
                                "local"
                            ) and not tts_format:
                                tts_encoding, tts_rate = "mulaw", 8000
                            tp = getattr(session, "transport_profile", None)
                    ...
                    if hangup_guardrail_enabled and tool_calls and any(tc.get("name") == "hangup_call" for tc in tool_calls):
                        ...
                        has_end_intent = (
                            ...
                            or text_is_short_polite_closing(transcript_text or "")
                            or text_contains_short_multilingual_farewell(transcript_text or "", max_words=12)
                        )
                        before_count = len(tool_calls)
                        if not has_end_intent:
                    ...
                    if not response_text and not tool_calls:
                        fb_raw = (llm_options or {}).get("empty_assistant_fallback") or (
                            llm_options or {}
                        ).get("empty_response_fallback")
                        fb = str(fb_raw or "").strip()
                        if fb:
                            response_text = fb
                            logger.info(
                                "Pipeline using empty-assistant spoken fallback",
                                call_id=call_id,
                                transcript_preview=(transcript_text or "")[:120],
                            )
                        else:
                            logger.warning(
                                "Pipeline turn skipped: LLM returned no text and no tool calls (no TTS)",
                                call_id=call_id,
                                transcript_preview=(transcript_text or "")[:120],
                    ...
                    if response_text:
                        ...
                        if use_streaming_playback:
                            ...
                            try:
                                ...
                                # End-of-segment sentinel (skip if barge-in already tore down the stream)
                                if call_id in self.streaming_playback_manager.active_streams:
                                    try:
                                        stream_q.put_nowait(None)
                                    except asyncio.QueueFull:
                            ...
                            try:
                                ...
                                if tool:
                                    ...
                                    if slow_threshold_ms > 0 and slow_message:
                                        ...
                                        if not done:
                                            try:
                                                await _pipeline_play_tts_audio(slow_message, "pipeline-wait")
                                            except Exception:
                                    ...
                                    if result.get("will_hangup"):
                                        ...
                                        if farewell:
                                            ...
                                            # Speak farewell (streaming path for AudioSocket)
                                            try:
                                                await _pipeline_play_tts_audio(farewell, "pipeline-farewell")
                                                logger.info("Farewell playback completed", call_id=call_id)
                                            except Exception as e:
                                        ...
                                        try:
                                            _fh_delay = float(
                                                getattr(self.config, "farewell_hangup_delay_sec", 2.5) or 2.5
                                            )
                                            await asyncio.sleep(_fh_delay)
                                            ...
                                            if llm_response:
                                                ...
                                                if getattr(llm_response, 'text', None):
                                                    ...
                                                    if response_text:
                                                        ...
                                                        await _pipeline_play_tts_audio(response_text, "pipeline-tts")
                                                        ...
                                                        if next_tool:
                                                            ...
                                                            if next_result.get("will_hangup"):
                                                                ...
                                                                try:
                                                                    ...
                                                                    logger.debug("Follow-up farewell TTS failed", call_id=call_id, exc_info=True)
                                                                _fd = float(
                                                                    getattr(self.config, "farewell_hangup_delay_sec", 2.5)
                                                                    or 2.5
                                                                )
                                                                await asyncio.sleep(_fd)
                                                                ch = getattr(session, "caller_channel_id", None) or call_id
                                                                await self.ari_client.hangup_channel(ch)
                                                                return
                                        except Exception as e:
                    ...
                    # Model often speaks a farewell without emitting hangup_call; ARI never hangs up.
                    # If this turn had no tools, user ended the call, and reply looks like goodbye — hang up.
                    if (
                        (not tool_calls)
                        and (response_text or "").strip()
                        and "hangup_call" in allowed_tools
                    ):
                        _hp = resolve_hangup_policy(getattr(self.config, "tools", None))
                        _mk = (_hp.get("markers") or {})
                        _end_m = _mk.get("end_call") or []
                        _fare_m = _mk.get("assistant_farewell") or []
                        _u = re.sub(r"\s+", " ", (transcript_text or "").strip().lower())
                        _a = re.sub(r"\s+", " ", (response_text or "").strip().lower())
                        _user_end = (
                            text_contains_end_call_intent(_u, _end_m)
                            or text_is_short_polite_closing(transcript_text or "")
                            or text_contains_short_multilingual_farewell(transcript_text or "", max_words=14)
                        )
                        _asst_farewell = text_contains_marker(
                            _a, _fare_m
                        ) or text_contains_short_multilingual_farewell(response_text or "", max_words=22)
                        _solo_terminal = assistant_short_terminal_farewell(response_text or "")
                        if (_user_end and _asst_farewell) or _solo_terminal:
                            _delay = float(getattr(self.config, "farewell_hangup_delay_sec", 2.5) or 2.5)
                            logger.info(
                                "Pipeline hangup heuristic: farewell without hangup_call",
                                call_id=call_id,
                                delay_sec=_delay,
                                user_end_intent=_user_end,
                                assistant_farewell=_asst_farewell,
                                assistant_solo_terminal=_solo_terminal,
                            )
                            try:
                                await asyncio.sleep(_delay)
                                _ch = getattr(session, "caller_channel_id", None) or call_id
                                await self.ari_client.hangup_channel(_ch)
                            except Exception:
                                logger.debug(
                                    "Pipeline hangup heuristic failed",
                                    call_id=call_id,
                                    exc_info=True,
                                )
                            return
    ...
    async def _assign_pipeline_to_session(
        ...
        if provider_override:
            if provider_override in self.providers:
                ...
                # Use "pipeline" so Calls UI shows pipeline and turn/latency/barge-in metrics.
                if session.provider_name != "pipeline":
                    session.provider_name = "pipeline"
                    updated = True
        else:
            # No primary_provider hint from pipeline; use "pipeline" so Calls UI shows pipeline and metrics.
            if session.provider_name != "pipeline":
                session.provider_name = "pipeline"
                updated = True

...
class TransportOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        audiosocket_config = config.get('audiosocket', {}) or {}
        # Outbound (wire) format: what we send to Asterisk (TTS). Inbound still uses format.
        wire_fmt = audiosocket_config.get('outbound_format') or audiosocket_config.get('format') or 'slin16'
        self.audiosocket_format = wire_fmt
        self.audiosocket_sample_rate = audiosocket_config.get('sample_rate', None)

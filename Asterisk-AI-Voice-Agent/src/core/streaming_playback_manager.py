...
class StreamingPlaybackManager:
    ...
    async def start_streaming_playback(
        ...
        try:
            ...
            # Source: prefer explicit; if missing and wire is slin, assume mulaw (telephony TTS)
            raw_src = self._canonicalize_encoding(source_encoding)
            if not raw_src and resolved_target_format in ("slin", "slin16"):
                raw_src = "ulaw"
            src_encoding = raw_src or "slin16"
            try:
    ...
    async def _stream_audio_loop(
        ...
        try:
            ...
            if stream_info is not None:
                ...
                er = str(stream_info.get('end_reason', '') or '').lower()
                # Caller interrupt: never drain jitter — that replays seconds of buffered TTS after barge-in.
                hard_stop = any(k in er for k in ('barge', 'interrupt', 'cancel'))
                ...
                if hard_stop:
                    ...
                    try:
                        while not jitter_buffer.empty():
                            try:
                                c = jitter_buffer.get_nowait()
                            except asyncio.QueueEmpty:
                                break
                            if c and c is not _JITTER_SENTINEL and isinstance(c, (bytes, bytearray)):
                                try:
                                    self._decrement_buffered_bytes(call_id, len(c))
                                except Exception:
                                    pass
                    except Exception:
                        logger.debug("Jitter purge after hard_stop failed", call_id=call_id, exc_info=True)
                    try:
                    ...
                    try:
                        frames_remaining = self._estimate_available_frames(
                            call_id, jitter_buffer, include_remainder=True
                        )
                    except Exception:
                        frames_remaining = 0
    ...
    async def _process_audio_chunk(self, call_id: str, chunk: bytes) -> Optional[bytes]:
        ...
        try:
            ...
            if (
                ...
                # When guard passes: always passthrough μ-law → μ-law (no decode/normalize/re-encode).
                # Normalizer was causing TTS noise when re-encoding; raw passthrough preserves quality.
                if guard_ok:
                    try:
                        logger.info(
                            "μ-law PASSTHROUGH - Sending raw TTS bytes (no processing)",
                            call_id=call_id,
                        ...
                        pass
                    return chunk
                # Guard failed: still prefer raw passthrough for ulaw→ulaw to avoid re-encode noise
                logger.warning(
                    "μ-law round-trip guard failed; passing through raw bytes anyway",
                    call_id=call_id,
                    chunk_bytes=len(chunk),
                )
                return chunk
        ...
        try:
            ...
            if self.audio_transport == "audiosocket":
                ...
                if not conn_id:
                    ...
                    return False
                # Apply PCM egress byte swap when Asterisk expects big-endian (slin/slin16)
                # force_true = always swap PCM; auto = swap for slin/slin16 (common fix for TTS noise)
                _fmt = self._canonicalize_encoding(target_fmt) or self._canonicalize_encoding(self.audiosocket_format) or "ulaw"
                _swap_mode = getattr(self, "egress_swap_mode", "disabled")
                _do_swap = (
                    len(chunk) > 0
                    and len(chunk) % 2 == 0
                    and _fmt in ("slin", "slin16", "linear16", "pcm16")
                    and (
                        _swap_mode == "force_true"
                        or (_swap_mode == "auto" and _fmt in ("slin", "slin16"))
                    )
                )
                if _do_swap:
                    try:
                        import audioop
                        chunk = audioop.byteswap(chunk, 2)
                    except Exception:
                        pass
    ...
    async def stop_streaming_playback(self, call_id: str) -> bool:
        ...
        try:
            ...
            # Cancel producer first and await it so _stream_audio_loop finally runs (handles barge-in
            # hard_stop: cancel pacer + purge jitter instead of draining seconds of buffered TTS).
            ...
            if task and not task.done():
                ...
                with suppress(asyncio.CancelledError):
                    await task
            # If the loop exited without cleanup (edge case), finish teardown here.
            if call_id in self.active_streams:
                try:
                ...
                if call_id in self.keepalive_tasks:
                    try:
                        kt = self.keepalive_tasks.pop(call_id, None)
                        if kt:
                            kt.cancel()
                            with suppress(asyncio.CancelledError):
                                await kt
        ...
        try:
            ...
            if self.conversation_coordinator:
                try:
                    ...
                    # Continuous stream: full-stream cleanup never runs until hangup, so without this
                    # conversation_state stays "greeting" and greeting_protection_ms applies to every segment.
                    sess = await self.session_store.get_by_call_id(call_id)
                    if sess and getattr(sess, "conversation_state", None) == "greeting":

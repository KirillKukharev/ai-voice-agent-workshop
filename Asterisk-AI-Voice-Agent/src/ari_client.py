        ...
        try:
            async with self.http_session.request(method, url, json=data, params=params) as response:
                if response.status >= 400:
                    ...
                    res = str(resource)
                    ch_var = "/channels/" in f"/{res}" and res.endswith("/variable")
                    method_u = str(method).upper()
                    # Benign GET: optional variable missing on a live channel.
                    if (
                    ...
                    # Benign GET/POST: channel already destroyed (hangup race) while reading/setting vars.
                    if (
                        int(response.status) == 404
                        and ch_var
                        and "Channel not found" in reason
                    ):
                        logger.debug(
                            "ARI channel variable skipped (channel gone)",
                            method=method,
                            url=url,
                            status=response.status,
                            reason=reason,

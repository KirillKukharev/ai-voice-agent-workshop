...
class AudioSocketConfig(BaseModel):
    ...
    format: str = Field(default="ulaw")  # Inbound: format Asterisk sends
    outbound_format: Optional[str] = Field(default=None)  # Outbound: format to send (TTS); else use format
...
class LocalProviderConfig(BaseModel):
    ...
    # TTS Backend selection: piper | kokoro | melotts | silero
...
class OpenAIProviderConfig(BaseModel):
    ...
    @field_validator("extra_http_headers", mode="before")
    @classmethod
    def _sanitize_extra_http_headers(cls, value: Any) -> Optional[Dict[str, str]]:
        """Drop null/empty header values so env-expanded YAML (e.g. Referer) does not break validation."""
        if value is None:
            return None
...
class BargeInConfig(BaseModel):
    ...
    # Voiced/unvoiced frames dip below RMS threshold every ~20ms; EMA + decay avoids never reaching min_ms.
    pipeline_barge_rms_ema_alpha: float = Field(default=0.22, ge=0.0, le=1.0)
    pipeline_barge_silent_decay_ratio: float = Field(default=0.4, ge=0.05, le=0.95)
    # Pipelines: also enable Asterisk TALK_DETECT when possible (extra signal on RTP bridges).
    # Engine still runs RMS-based barge-in during TTS gating — AudioSocket often gets no TalkDetect events.
...
def _normalize_pipelines(config_data: Dict[str, Any]) -> None:
    ...
    def _component_default_provider(provider_hint: Any, dp: Any) -> str:
        if provider_hint:
            return str(provider_hint).strip()
        dps = str(dp or "").strip()
        if isinstance(pipelines_cfg, dict) and dps in pipelines_cfg:
            return "local"
        if "_" in dps and dps not in _slug:
            return "local"
        return dps or "openai_realtime"

    for pipeline_name, raw_entry in pipelines_cfg.items():
        if raw_entry is None:
            normalized[pipeline_name] = _compose_provider_components(
                _component_default_provider(None, default_provider)
            )
            continue
        ...
        if isinstance(raw_entry, dict):
            ...
            provider_for_defaults = _component_default_provider(provider_hint, default_provider)
...
def validate_production_config(config: AppConfig) -> tuple[list[str], list[str]]:
    ...
    try:
        ...
        # Binding exposure warnings (skip when advertise_host or ASTERISK_HOST set — intentional Docker/NAT)
        try:
            if hasattr(config, 'audiosocket') and config.audiosocket:
                if getattr(config.audiosocket, 'host', None) == '0.0.0.0':
                    adv = getattr(config.audiosocket, 'advertise_host', None) or ''
                    asterisk_host = (os.getenv('ASTERISK_HOST') or '').strip()
                    if adv not in ('0.0.0.0', '::', '') or (asterisk_host and asterisk_host not in ('127.0.0.1', 'localhost', '')):
                        pass  # Docker/NAT: advertise_host or remote Asterisk — no warning
                    else:

...
def _parse_tool_call_arguments(raw: Any) -> Dict[str, Any]:
    """Parse OpenAI-style function.arguments (string JSON or already-decoded dict)."""
    ...
    try:
        ...
        logger.warning("Tool call arguments JSON parse failed", preview=s[:160])
        return {}
...
def _normalize_assistant_content(content: Any) -> str:
    """Chat Completions `message.content` may be a string, null, or a list of parts (multimodal)."""
    if content is None:
        return ""
...
def _sanitize_openrouter_reasoning(raw: Any) -> Optional[Dict[str, Any]]:
    """Build OpenRouter `reasoning` object (effort vs max_tokens — not both required; we pass through valid keys)."""
    if not isinstance(raw, dict) or not raw:
        return None
    out: Dict[str, Any] = {}
    if raw.get("effort") is not None:
        e = str(raw["effort"]).strip().lower()
        if e in ("xhigh", "high", "medium", "low", "minimal", "none"):
            out["effort"] = e
...
class OpenAILLMAdapter(LLMComponent):
    ...
    async def validate_connectivity(self, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # If options contain an incompatible model, don't let it override validation — only for native OpenAI host.
    ...
    async def generate(
        ...
        reasoning_salvage_attempted = False
        openrouter_reasoning_retry = False
            ...
            try:
                async with self._session.post(url, json=payload, headers=headers, timeout=merged["timeout_sec"]) as response:
                    ...
                    if response.status >= 400:
                        ...
                        # OpenRouter / StepFun: "Reasoning is mandatory ... cannot be disabled" when effort is none.
                        if (
                            not openrouter_reasoning_retry
                            and response.status == 400
                            and "openrouter.ai" in str(url).lower()
                            and (
                                "Reasoning is mandatory" in body
                                or "cannot be disabled" in body
                            )
                            and attempt < retries
                        ):
                            openrouter_reasoning_retry = True
                            payload = dict(payload)
                            payload["reasoning"] = {"effort": "low"}
                            logger.warning(
                                "OpenRouter rejected disabled/missing reasoning; retrying with reasoning.effort=low",
                                call_id=call_id,
                            )
                            continue
                        ...
                        # Free-tier / busy providers often return 429; immediate retries rarely help without backoff.
                        if response.status in (429, 503) and attempt < retries:
                            wait_s = min(120.0, max(1.0, 2.0 * (2**attempt)))
                            ra_hdr = response.headers.get("Retry-After")
                            if ra_hdr:
                                try:
                                    wait_s = min(120.0, max(1.0, float(ra_hdr)))
                                except (TypeError, ValueError):
                                    pass
                            logger.warning(
                                "Chat completion rate limited or service unavailable; backing off before retry",
                                call_id=call_id,
                                status=response.status,
                                wait_sec=round(wait_s, 2),
                                attempt=attempt + 1,
                            )
                    ...
                    content = _normalize_assistant_content(raw_content)
                    tool_calls = message.get("tool_calls") or []
                    finish_reason = choices[0].get("finish_reason")
                    native_fr = choices[0].get("native_finish_reason")
                        ...
                        for tc in tool_calls:
                            try:
                                ...
                                parsed_tool_calls.append({
                                    "id": tc.get("id"),
                                    "name": name,
                                    "parameters": _parse_tool_call_arguments(args),
                    ...
                    # Reasoning-heavy models (e.g. Nemotron via OpenRouter) may return finish_reason=length with
                    # non-empty `reasoning` but empty `content`. Retry once with reasoning disabled + more headroom.
                    if (
                        not tool_calls
                        and not content.strip()
                        and not reasoning_salvage_attempted
                        and str(finish_reason or "").lower() == "length"
                        and isinstance(message, dict)
                    ):
                        r_blob = message.get("reasoning")
                        rd_blob = message.get("reasoning_details")
                        has_reasoning = (
                            (isinstance(r_blob, str) and r_blob.strip() != "")
                            or (r_blob not in (None, "", [], {}))
                            or (rd_blob not in (None, "", [], {}))
                        )
                        if has_reasoning:
                            reasoning_salvage_attempted = True
                            payload = dict(payload)
                            payload["reasoning"] = {"effort": "none"}
                            try:
                                mt = int(payload.get("max_tokens") or 0)
                                if mt > 0:
                                    payload["max_tokens"] = min(int(mt * 1.5) + 128, 4096)
                            except (TypeError, ValueError):
                                pass
                            logger.warning(
                                "Retrying chat completion: reasoning consumed output budget (empty content)",
                                call_id=call_id,
                                model=payload.get("model"),
                            )
                            continue
                    ...
                    if not content.strip() and not tool_calls:
                        logger.warning(
                            "OpenAI chat completion returned empty assistant text",
                            call_id=call_id,
                            model=payload.get("model"),
                    ...
                    if not tool_calls and payload.get("tools"):
                        logger.warning(
                            "Chat completion returned assistant text but no tool_calls while tools were "
                            "offered; in-call tools (e.g. google_calendar) will not run. Use an LLM that "
                            "supports OpenAI-style function calling (many free OpenRouter models omit tool_calls).",
                            call_id=call_id,
                            model=payload.get("model"),
                            finish_reason=finish_reason,
                    ...
                    return LLMResponse(text=content, tool_calls=[], metadata=data.get("usage", {}))
    ...
    def _compose_options(self, runtime_options: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        merged = {
            ...
            "reasoning": _sanitize_openrouter_reasoning(
                runtime_options.get("reasoning", self._pipeline_defaults.get("reasoning"))
            ),
        ...
        # YAML often sets `model: null` to mean "use provider default". dict.get("model", default) still
        # returns None when the key exists, which breaks OpenRouter/OpenAI chat payloads.
        def _nonempty_model_val(src: Optional[Dict[str, Any]], *keys: str) -> Optional[str]:
        ...
        if not _nonempty_model_val(merged, "chat_model"):
            resolved = _nonempty_model_val(runtime_options or {}, "chat_model", "model") or _nonempty_model_val(
                self._pipeline_defaults or {}, "chat_model", "model"
            )
            if not resolved:
                prov_m = getattr(self._provider_defaults, "chat_model", None)
                if prov_m is not None and str(prov_m).strip():
                    resolved = str(prov_m).strip()
            merged["chat_model"] = resolved or "gpt-4o-mini"
        ...
        if _url_host(chat_base) == "api.groq.com" or chat_model.startswith("llama-") or "mixtral" in chat_model:
            ...
            fb_model = getattr(self._provider_defaults, "chat_model", None)
            merged["chat_model"] = (
                str(fb_model).strip()
                if fb_model is not None and str(fb_model).strip()
                else "gpt-4o-mini"
            )

        # Last resort: OpenRouter rejects requests with missing/null model (error: "No models provided").
        if not str(merged.get("chat_model") or "").strip():
            merged["chat_model"] = "gpt-4o-mini"
        ...
        # Do not default OpenRouter to reasoning.effort=none: some routed models (e.g. StepFun) return 400
        # ("Reasoning is mandatory ... cannot be disabled"). Omit `reasoning` from the payload when unset;
        # YAML may still set effort none/none for models that need it, with a 400→low retry in generate().
        return merged

    def _build_chat_payload(self, transcript: str, context: Dict[str, Any], merged: Dict[str, Any]) -> Dict[str, Any]:
        messages = self._coalesce_messages(transcript, context, merged)
        raw_m = merged.get("chat_model")
        use_model = str(raw_m).strip() if raw_m is not None else ""
        if not use_model or use_model.lower() in {"null", "none"}:
            pm = getattr(self._provider_defaults, "chat_model", None)
            use_model = str(pm).strip() if pm is not None and str(pm).strip() else "gpt-4o-mini"
            merged["chat_model"] = use_model

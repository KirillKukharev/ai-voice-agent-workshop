...
from ..core.tts_speakable_ru import normalize_russian_tts_text
...
class _LocalAdapterBase:
    ...
    async def _send_json(self, session: _LocalSessionState, payload: Dict[str, Any]) -> None:
        try:
            await session.websocket.send(json.dumps(payload, ensure_ascii=False))
        except Exception as exc:
            logger.error(
...
class LocalTTSAdapter(_LocalAdapterBase, TTSComponent):
    ...
    async def synthesize(
        ...
        merged = self._compose_options(runtime_options)
        tts_text = normalize_russian_tts_text(text)
        if tts_text != text:
            logger.debug(
                "TTS text normalized for Russian speech",
                component=self.component_key,
                call_id=call_id,
                preview=(tts_text or "")[:100],
            )
        logger.debug(
            "Sending TTS request",
            component=self.component_key,
            call_id=call_id,
            text_preview=(tts_text or "")[:80],
        )
        payload = {
            "type": "tts_request",
            "call_id": call_id,

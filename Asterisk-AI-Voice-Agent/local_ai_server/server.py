...
# Filters: empty connections (0 bytes), "opening handshake failed", "did not receive a valid HTTP request"
class _WebSocketHandshakeFilter(logging.Filter):
    """Filter out noisy websockets handshake failures from health checks/probes/incomplete connections."""
    _NOISE_PHRASES = (
    ...
    def filter(self, record):
        # Only apply to websockets loggers
        if not (getattr(record, "name", "") or "").startswith("websockets"):
            return True
        ...
        # Suppress if any noise phrase appears in message or in raw msg (format string)
        return not any(phrase in msg for phrase in self._NOISE_PHRASES)


def _install_websocket_handshake_filter():
    """Install handshake filter on both websockets logger and root so it always runs."""
    f = _WebSocketHandshakeFilter()
    logging.getLogger("websockets").addFilter(f)
    logging.getLogger("websockets.server").addFilter(f)
    # Also attach to root so any handler (e.g. from uvicorn/docker) sees the filter
    root = logging.getLogger()
    root.addFilter(f)


_install_websocket_handshake_filter()
...
class LocalAIServer:
    ...
    def _apply_config(self, config: LocalAIConfig) -> None:
        ...
        self.silero_tts_ulaw_output = config.silero_tts_ulaw_output
        self.silero_tts_delivery_sample_rate = config.silero_tts_delivery_sample_rate
    ...
    async def process_llm_chat(self, messages: List[Dict[str, Any]]) -> str:
    ...
    async def _process_tts_silero(self, text: str) -> bytes:
        """Process TTS using Silero backend (μ-law 8 kHz telephony or raw PCM16 for wideband)."""
        try:
            ...
            if not self.silero_tts_ulaw_output:
                out_rate = self.silero_tts_delivery_sample_rate or self.silero_sample_rate
                if out_rate > 0 and out_rate != self.silero_sample_rate:
                ...
                logging.info(
                    "🔊 TTS RESULT - Silero raw PCM16 %dHz audio: %s bytes",
                    out_rate,
                    len(pcm16_data),
                )
                return pcm16_data

            # Write to temp WAV file for μ-law conversion
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as wav_file:
                wav_path = wav_file.name
    ...
    def _normalize_llm_content_for_chat(self, value: Any) -> str:
        """Coerce OpenAI-style message content to a string for llama.cpp chat completion."""
        if value is None:
            return ""
    ...
    def _prior_turns_from_pipeline_llm_context(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Extract prior dialog turns from pipeline ``llm_request`` context (``prior_messages``)."""
        ctx = data.get("context")
        candidates: List[Any] = []
        if isinstance(ctx, list):
            candidates = list(ctx)
        elif isinstance(ctx, dict):
            raw = ctx.get("messages")
            if isinstance(raw, list):
                candidates = list(raw)
            else:
                raw2 = ctx.get("prior_messages")
                if isinstance(raw2, list):
                    candidates = list(raw2)

        allowed = frozenset({"user", "assistant", "tool"})
        ...
        for m in candidates:
            ...
            if role not in allowed:
                continue
            body = self._normalize_llm_content_for_chat(m.get("content"))
            if role == "assistant" and not body.strip():
            ...
            if role == "tool":
                ...
                if not tid:
                    continue
                out.append({"role": "tool", "content": body, "tool_call_id": str(tid)})
                continue
    ...
    async def _handle_llm_request(
        ...
        try:
            ...
            if use_chat_path:
                ...
                prior = self._prior_turns_from_pipeline_llm_context(data)
                if prior:
                    logging.info(
                        "🧠 LLM CONTEXT - Merging %d prior turn(s) from pipeline request call_id=%s",
                        len(prior),
                        session.call_id,
                    )
                chat_msgs.extend(prior)

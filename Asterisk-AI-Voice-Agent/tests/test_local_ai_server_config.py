...
def test_silero_config_defaults(monkeypatch):
    ...
    monkeypatch.delenv("SILERO_TTS_ULAW_OUTPUT", raising=False)
    monkeypatch.delenv("SILERO_TTS_DELIVERY_SAMPLE_RATE", raising=False)
    config_mod = _load_local_ai_config_module()
    cfg = config_mod.LocalAIConfig.from_env()
    ...
    assert cfg.silero_tts_ulaw_output is True
    assert cfg.silero_tts_delivery_sample_rate is None
...
def test_silero_config_from_env(monkeypatch):
    ...
    monkeypatch.setenv("SILERO_TTS_ULAW_OUTPUT", "false")
    monkeypatch.setenv("SILERO_TTS_DELIVERY_SAMPLE_RATE", "16000")
    config_mod = _load_local_ai_config_module()
    cfg = config_mod.LocalAIConfig.from_env()
    ...
    assert cfg.silero_tts_ulaw_output is False
    assert cfg.silero_tts_delivery_sample_rate == 16000

...
def download_short_audio():
    ...
    if not Path(audio_file).exists():
        os.system('wget -O example.wav "https://cdn.chatwm.opensmodel.sberdevices.ru/GigaAM/example.wav"')
    assert Path(audio_file).exists(), "Short audio file not found"
    return audio_file
...
def download_long_audio():
    ...
    if not Path(audio_file).exists():
        os.system('wget -O long_example.wav "https://cdn.chatwm.opensmodel.sberdevices.ru/GigaAM/long_example.wav"')
    assert Path(audio_file).exists(), "Long audio file not found"
    return audio_file
...
class AudioDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, lst: list[str | np.ndarray | torch.Tensor]):
        ...
        assert isinstance(lst[0], (str, np.ndarray, torch.Tensor)), f"Unexpected dtype: {type(lst[0])}"  # noqa: UP038
        self.lst = lst

...
_CACHE_DIR = Path("~/.cache/gigaam").expanduser()
...
def _download_file(file_url: str, file_path: str):
    ...
    with (
        ...
        Path.open(file_path, "wb") as output,
        tqdm(
...
def _download_model(model_name: str, download_root: str) -> tuple[str, str]:
    ...
    model_path = Path(download_root).join(model_name + ".ckpt")
...
def _download_tokenizer(model_name: str, download_root: str) -> str | None:
    ...
    tokenizer_path = Path(download_root).join(model_name + "_tokenizer.model")
    return _download_file(tokenizer_url, tokenizer_path)


def hash_path(ckpt_path: str) -> str:
    """Calculate binary file hash for checksum"""
    return hashlib.md5(Path.open(ckpt_path, "rb").read()).hexdigest()

...
class GigaAMASR(GigaAM):
    ...
    @torch.inference_mode()
    def transcribe_longform(self, wav_file: str, **kwargs) -> list[dict[str, str | tuple[float, float]]]:
        """
        ...
        for segment, segment_boundaries in zip(segments, boundaries, strict=False):
            wav = segment.to(self._device).unsqueeze(0).to(self._dtype)
            length = torch.full([1], wav.shape[-1], device=self._device)
            encoded, encoded_len = self.forward(wav, length)
            result = self.decoding.decode(self.head, encoded, encoded_len)[0]
            transcribed_segments.append({"transcription": result, "boundaries": segment_boundaries})

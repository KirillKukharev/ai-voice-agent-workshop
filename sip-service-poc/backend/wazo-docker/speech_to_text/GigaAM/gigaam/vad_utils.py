...
def resolve_local_segmentation_path(model_id: str) -> str:
    ...
    if not hf_token:
        raise RuntimeError(f"Model {model_id} was not found locally, and no HF_TOKEN was provided to download it.")
...
def get_pipeline(device: torch.device, model_id: str = "pyannote/segmentation-3.0") -> Pipeline:
    """
